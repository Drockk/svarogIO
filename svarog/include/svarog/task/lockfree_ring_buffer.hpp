...
public:
    lockfree_ring_buffer_t(uint32_t power_of_2_size)
        : m_capacity(next_power_of_2(power_of_2_size)), m_power_of_2_mod(m_capacity - 1), buffer(m_capacity, nullptr) {
    ...
    void* try_pop() {
        ...
        if (ret && high > low && this->m_low.compare_exchange_weak(low, low + 1, std::memory_order_release)) {
