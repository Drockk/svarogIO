...
public:
    ...
    task_list_o(size_t t_capacity_hint = 32)
        : m_tasks(t_capacity_hint), m_num_tasks(0)
    {
    }
    ...
    {
        m_tasks.unsafe_for_each([](void* c, void* p_scheduler)
            {
...
TaskList::TaskList(size_t t_hint_capacity)
    : m_impl(new task_list_o(t_hint_capacity))
{
}
...
public:
    ...
    ~scheduler_impl()
    {
        ...
        for(auto* channel: m_channels) {
            if(channel) {
                channel->flag.test_and_set();   // Set flag so that if there is a worker blocked on this flag, it may proceed.
                channel->flag.notify_one();     // Notify the worker thread (if any worker thread is waiting) that the flag has flipped.
        ...
        while (t_task_list.m_impl->get_tasks_count()) {
            ...
            if (c == nullptr) {
                ...
                if (t_task_list.m_impl->block_flag.test_and_set(std::memory_order::acq_rel)) {
                    std::cout << "blocking thread " << std::this_thread::get_id() << " on [" << t_task_list.m_impl << "]" << std::endl;
                    ...
                    std::cout << "resuming thread " << std::this_thread::get_id() << " on [" << t_task_list.m_impl << "]" << std::endl;
                } else {
                    std::cout << "spinning thread " << std::this_thread::get_id() << " on [" << t_task_list.m_impl << "]" << std::endl;
                }
...
Scheduler::Scheduler(const size_t t_number_worker_thread)
    : m_impl(new scheduler_impl(t_number_worker_thread))
{
}
